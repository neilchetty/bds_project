...
IMAGE_TAG="${2:-gene2life-java:latest}"
CONTAINER_PREFIX="g2l-pincheck"

cleanup() {
  local containers
  containers="$(docker ps -aq --filter "label=org.gene2life.runtime=pincheck")"
...
while IFS= read -r line; do
  ...
  docker run -d --rm \
    --name "$container_name" \
    --label org.gene2life.runtime=pincheck \
    --cpus "$cpu_threads" \
