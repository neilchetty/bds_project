...
IMAGE_TAG="${4:-gene2life-hadoop-cluster:3.4.3}"
...
WARMUP_ON_START="${HADOOP_CLUSTER_WARMUP_ON_START:-true}"
...
labels_ready() {
  ...
  compose exec -T master bash -lc "
    yarn cluster --list-node-labels >/tmp/gene2life-labels.txt 2>/tmp/gene2life-labels.err || exit 1
    labels='${labels}'
    IFS=',' read -r -a expected <<< \"\$labels\"
    for label in \"\${expected[@]}\"; do
      grep -q \"<\${label}:exclusivity=true>\" /tmp/gene2life-labels.txt || exit 1
    done
  " >/dev/null 2>&1
}
...
wait_for_health() {
  ...
  for _ in $(seq 1 90); do
    if compose exec -T master bash -lc "hdfs dfsadmin -report >/tmp/hdfs-report.txt 2>/tmp/hdfs-report.err && yarn node -list >/tmp/yarn-nodes.txt 2>/tmp/yarn-nodes.err"; then
      local current_nodes
...
generate_compose() {
  "$ROOT_DIR/scripts/generate-hadoop-compose.sh" "$CLUSTER_CONFIG" "$OUTPUT_DIR" "$IMAGE_TAG"
}

init_hdfs_dirs() {
  compose exec -T master bash -lc "hdfs dfs -mkdir -p /tmp /user/${CLIENT_USER} /user/${CLIENT_USER}/.staging /user/${CLIENT_USER}/gene2life \
...
validate_cluster() {
  ...
  validation_root="$(mktemp -d "${TMPDIR:-/tmp}/gene2life-hadoop-validate.XXXXXX")"
  trap "rm -rf '$validation_root'" RETURN
  compose exec -T master bash -lc "printf 'gene2life-validation\n' > /tmp/gene2life-validate.txt && hdfs dfs -put -f /tmp/gene2life-validate.txt /user/${CLIENT_USER}/gene2life/validate.txt && hdfs dfs -cat /user/${CLIENT_USER}/gene2life/validate.txt"
  ...
  compose exec -T master bash -lc "hdfs dfs -rm -f /user/${CLIENT_USER}/gene2life/validate.txt >/dev/null 2>&1 || true"
  echo "Validation run completed under $validation_root/heft"
}
